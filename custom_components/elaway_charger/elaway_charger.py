"""MQTT integration for Elaway EV Charger in Home Assistant."""
...
# Charger API URLs (configure in config entry)
CHARGER_API_URL = ""
CHARGER_START_URL = ""
CHARGER_STOP_URL = ""
...
class ElawayChargerIntegration:
    """Integration for Elaway EV Charger."""
    ...
    async def publish_discovery(self) -> None:
        ...
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/config",
        ...
        await mqtt.async_publish(
            self.hass,
            "homeassistant/button/charger_start/config",
            json.dumps(switch_start_config),
            retain=True
        )
        ...
        await mqtt.async_publish(
            self.hass,
            "homeassistant/button/charger_stop/config",
            json.dumps(switch_stop_config),
            retain=True
        )
        
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/fixedfee/config",
            json.dumps(fixedfee_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/sessionenergy/config",
            json.dumps(sessionenergy_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/sessionpower/config",
            json.dumps(sessionpower_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/sessiontotal/config",
            json.dumps(sessiontotal_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/timestarted/config",
            json.dumps(timestarted_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/tariff/config",
            json.dumps(tariff_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/sensor/charger/monthenergy/config",
            json.dumps(monthenergy_config),
            retain=True
        )
        await mqtt.async_publish(
            self.hass,
            "homeassistant/binary_sensor/charger/available/config",
            json.dumps(available_config),
            retain=True
        )
        ...
        try:
            async with self._session.get(self.charger_api_url) as resp:
                ...
                try:
                    ...
                    await mqtt.async_publish(
                        self.hass,
                        f"{MQTT_TOPIC_SESSIONENERGY}/state",
                        sessionenergy / 1000  # Convert to kWh
                    )
                except KeyError:
                    await mqtt.async_publish(
                        self.hass,
                ...
                try:
                    ...
                    await mqtt.async_publish(
                        self.hass,
                        f"{MQTT_TOPIC_SESSIONPOWER}/state",
                        sessionpower
                    )
                except KeyError:
                    await mqtt.async_publish(
                ...
                try:
                    ...
                    await mqtt.async_publish(
                        self.hass,
                        f"{MQTT_TOPIC_SESSIONTOTAL}/state",
                        session_total
                    )
                except KeyError:
                    await mqtt.async_publish(
                ...
                try:
                    ...
                    await mqtt.async_publish(
                        self.hass,
                        f"{MQTT_TOPIC_TIMESTARTED}/state",
                        time_started
                    )
                except KeyError:
                    await mqtt.async_publish(
                ...
                try:
                    ...
                    await mqtt.async_publish(
                        self.hass,
                        f"{MQTT_TOPIC_MONTHENERGY}/state",
                        monthenergy
                    )
                except KeyError:
                ...
                try:
                    ...
                    await mqtt.async_publish(
                        self.hass,
                        f"{MQTT_TOPIC_BINARY_SENSOR_STATUS}/state",
                        payload
                    )
                except KeyError:
